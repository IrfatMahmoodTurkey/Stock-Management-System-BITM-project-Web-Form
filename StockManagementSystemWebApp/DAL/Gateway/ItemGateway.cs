...
namespace StockManagementSystemWebApp.DAL.Gateway
{
    public class ItemGateway:CommonGateway
    {
        ...
        public int Save(Items item)
        {
            ...
            Command.Parameters.Add("@categoryId", item.CategoryId);
            Command.Parameters.Add("@companyId", item.CompanyId);
            Command.Parameters.Add("@itemName", item.ItemName);
            Command.Parameters.Add("@reorderLevel", item.ReorderLevel);
            Command.Parameters.Add("@availableQuantity", item.AvailableQuantity);
        ...
        public Items GetItemByItemAndCompany(int companyId, int itemId)
        {
            ...
            Command = new SqlCommand(query, Connection);
            Command.Parameters.Add("@companyId", companyId);
            Command.Parameters.Add("@id", itemId);
        ...
        public int UpdateAvailableQuantityByItemIdAndCompany(StockIn stockIn, int totalQuantity)
        {
            ...
            Command = new SqlCommand(query, Connection);
            Command.Parameters.Add("@availableQuantity", totalQuantity);
            Command.Parameters.Add("@id", stockIn.ItemId);
        ...
        public int UpdateAvailableQuantityByItemIdAndCompanyOfStockOut(StockOut stockOut, int totalQuantity)
        {
            ...
            Command = new SqlCommand(query, Connection);
            Command.Parameters.Add("@availableQuantity", totalQuantity);
            Command.Parameters.Add("@id", stockOut.ItemId);
        ...
        public List<ItemViewModel> GetItem()
        {
            ...
            string query =
                "SELECT CategoryName as Category, CompanyName as Company, ItemName, ReorderLevel, AvailableQuantity FROM Items INNER JOIN Company ON Items.CompanyId = Company.Id INNER JOIN Category ON Items.CategoryId = Category.Id";
            Command = new SqlCommand(query, Connection);
            Reader = Command.ExecuteReader();
